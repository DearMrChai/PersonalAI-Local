// services/chat/llm/providers/llama-provider.js
...
export class LlamaProvider extends LlmProvider {
  ...
  async init() {
    ...
    try {
      console.log('⏳ [LlamaProvider] 正在加载 Llama-3-8B 模型...');
      ...
      this.model = await this.llama.loadModel({
        ...
        contextSize: 4096,
        threads: 6,
        gpuLayers: 10,           // 8B 模型建议范围 15~30
      });

      console.log('✅ [LlamaProvider] Llama-3-8B 模型加载成功');
    } catch (error) {
      console.error('❌ [LlamaProvider] 模型加载失败:', error.message);
      throw error;
    }
  }

  /**
   * 流式生成回复 - 基于你最初能正常工作的写法
   */
  /**
   * 流式生成回复 - 适配 node-llama-cpp v3+ API
   */
  async *streamGenerate(messages, options = {}) {
    try {
      ...
      const sessionPromise = session.prompt(lastUserMsg, {
        ...
        topK: 40,
        onTextChunk: (chunk) => {
          console.log('[LlamaProvider] onTextChunk callback triggered2改', chunk);
          chunks.push(chunk);
          if (notify) {
            notify();
            notify = null;
